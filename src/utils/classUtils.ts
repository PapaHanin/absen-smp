...
export const STANDARD_SMP_CLASSES = [
  'Kelas 7A',
  'Kelas 7B',
  'Kelas 8A',
  'Kelas 8B',
...
export const STANDARD_SD_CLASSES = STANDARD_SMP_CLASSES;

/**
 * Returns available classes from system settings or fallback to standard SMP classes
 */
export function getAvailableClasses(customClasses?: string[]): string[] {
  if (customClasses && Array.isArray(customClasses) && customClasses.length > 0) {
    return customClasses;
  }
  return STANDARD_SMP_CLASSES;
}

/**
 * Normalizes class strings for safe comparison
 * e.g. "Kelas 7A" -> "7a", "kelas 7-A" -> "7-a", "7" -> "7"
 */
 ...
 * Matches:
 * - "Kelas 7A" === "7a" === "Kelas 7A"
 * - "Kelas 7" matches student in "7", "Kelas 7", "7A", "7B", "7-A"
 * - "7A" strictly matches "7A" or "Kelas 7A"
 */
export function isHomeroomClassMatch(studentClass?: string, homeroomClass?: string): boolean {
  ...
  // If homeroom is general grade (e.g. 7, 8, 9 or 1-6)
  if (['7', '8', '9', '1', '2', '3', '4', '5', '6'].includes(hrClean)) {
    if (stdClean === hrClean) return true;
    if (
      stdClean.startsWith(`${hrClean}-`) ||
      stdClean.startsWith(`${hrClean}_`) ||
      stdClean.startsWith(hrClean)
    ) {
      return true;
    }
