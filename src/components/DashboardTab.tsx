  ...
  const filteredTableData = useMemo(() => {
    return dateFilteredRecords.filter((rec) => {
      ...
      if (selectedTypeFilter !== 'Semua') {
        matchType = selectedTypeFilter === 'mapel' ? rec.attendanceType === 'mapel' : rec.attendanceType !== 'mapel';
      }

      return matchSearch && matchClass && matchStatus && matchTeacher && matchType;
    });
  }, [dateFilteredRecords, searchTerm, activeClass, selectedStatus, selectedTeacherFilter, selectedTypeFilter, teachers, students, currentTeacher]);
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-4 sm:p-5 shadow-xs space-y-4 transition-colors">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* Filter Tipe Presensi (Harian vs Mapel SMP) */}
            <div className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-1.5 text-xs">
              <i className="fa-solid fa-book-open-reader text-blue-600 dark:text-blue-400 text-xs"></i>
              <span className="font-semibold text-slate-500 dark:text-slate-400 hidden sm:inline">Tipe:</span>
              <select
                value={selectedTypeFilter}
                onChange={(e) => setSelectedTypeFilter(e.target.value as 'Semua' | 'harian' | 'mapel')}
                className="bg-transparent text-slate-800 dark:text-slate-100 font-bold focus:outline-none cursor-pointer"
              >
                <option value="Semua" className="bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100">Semua Tipe</option>
                <option value="harian" className="bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100">Presensi Harian / Gerbang</option>
                <option value="mapel" className="bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100">Presensi Jam Mapel SMP</option>
              </select>
            </div>

            {/* Shortcut Button Atur Jadwal Jam Pelajaran SMP */}
            {onOpenLessonSchedule && (
              <button
                type="button"
                onClick={onOpenLessonSchedule}
                className="flex items-center gap-1.5 bg-blue-50 dark:bg-blue-950/60 hover:bg-blue-100 dark:hover:bg-blue-900/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 rounded-xl px-3 py-1.5 text-xs font-bold transition-colors cursor-pointer"
                title="Kelola Jam Pelajaran & Jadwal Guru Mapel SMP"
              >
        ...
        <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 border-t border-slate-100 dark:border-slate-800 pt-3">
          ...
          {(searchTerm || selectedClass !== 'Semua' || selectedStatus !== 'Semua' || selectedTeacherFilter !== 'Semua' || selectedTypeFilter !== 'Semua') && (
            <button
              onClick={() => {
                setSearchTerm('');
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 uppercase tracking-wider text-[10px] font-bold border-b border-slate-200 dark:border-slate-700">
                <tr>
                  ...
                  <th className="py-3 px-4">Tipe Presensi</th>
                        ...
                        <td className="py-3 px-4">
                          {record.attendanceType === 'mapel' ? (
                            <div className="flex flex-col">
                              <span className="inline-flex items-center gap-1 text-[11px] font-extrabold text-blue-700 dark:text-blue-300">
                                <i className="fa-solid fa-book-bookmark text-[10px]"></i>
                                {record.subject || 'Mapel'}
                              </span>
                              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">
                                {record.periodName || (record.periodNumber ? `Jam ke-${record.periodNumber}` : 'Jam Mapel')}
                              </span>
                            </div>
                          ) : (
