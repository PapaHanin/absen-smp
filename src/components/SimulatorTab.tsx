...
export const SimulatorTab: React.FC<SimulatorTabProps> = ({
  ...
  // Dynamic School Classes Management
  const [classList, setClassList] = useState<string[]>(() => {
    if (settings.customClasses && Array.isArray(settings.customClasses) && settings.customClasses.length > 0) {
      return settings.customClasses;
    }
  ...
  useEffect(() => {
    ...
    if (settings.customClasses && Array.isArray(settings.customClasses) && settings.customClasses.length > 0) {
      setClassList(settings.customClasses);
    }
  }, [settings]);
  ...
  const handleAddClass = () => {
    ...
    if (!formatted.toLowerCase().startsWith('kelas')) {
      formatted = `Kelas ${formatted}`;
    }
    if (classList.includes(formatted)) {
      alert(`Kelas ${formatted} sudah ada dalam daftar.`);
      return;
    }
    const updated = [...classList, formatted].sort((a, b) =>
  ...
  const handleRemoveClass = (classToRemove: string) => {
    if (classList.length <= 1) {
      alert('Sekolah harus memiliki minimal 1 kelas aktif.');
      return;
    }
    if (confirm(`Hapus ${classToRemove} dari daftar rombel sekolah?`)) {
      const updated = classList.filter((c) => c !== classToRemove);
      setClassList(updated);
      onUpdateSettings({
        ...settings,
        customClasses: updated,
            ...
            <form onSubmit={handleSaveSettings} className="space-y-3">
              ...
              <div className="p-3.5 rounded-2xl bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/60 space-y-2.5">
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400">
                  Tambah atau hapus kelas sesuai kebutuhan sekolah (misal hanya Kelas 7, 8, 9, atau tambah 7B, 8B, 9B).
                </p>

                {/* Badges of current classes with delete button */}
                    ...
                    >
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="text-slate-400 hover:text-rose-600 ml-0.5 p-0.5 transition-colors cursor-pointer"
                          title={`Hapus ${cls}`}
                        >
                ...
                {/* Input to add new class */}
                {isAdmin && (
                  <div className="flex items-center gap-2 pt-1.5">
                    <input
                      type="text"
                      placeholder="Nama Kelas baru (contoh: 7B, Kelas 7B, Kelas 8...)"
                      value={newClassInput}
