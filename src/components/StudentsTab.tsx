...
export const StudentsTab: React.FC<StudentsTabProps> = ({
  ...
  // Dynamic unique classes list derived from custom settings classes, actual students & fallback SMP classes
  const availableClasses = useMemo(() => {
    const classSet = new Set<string>();
    const baseClasses = (settings.customClasses && settings.customClasses.length > 0)
      ? settings.customClasses
      : SD_CLASSES;
    baseClasses.forEach((c) => classSet.add(formatClassLabel(c)));
    students.forEach((s) => {
  ...
  }, [students, settings.customClasses]);
          ...
          {!isGuruMapel && (
            <>
              <button
                ...
                title="Pilih dan unggah file Excel (.xls, .xlsx) untuk memperbarui data siswa secara otomatis"
              >
